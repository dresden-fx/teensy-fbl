...
void bmgr_execWaitCmd(T_BMGR_DATA* bmgrData)
{
...
//  TRACE_BMGR_API("bmgr_execWaitCmd()\n");
  /* Check timeout */
  ...
  {
    /* No message was received, still waiting for bootstrap message. */
  }
}


void bmgr_execSendAck(T_BMGR_DATA* bmgrData)
{
  T_BCP_STATUS bcpStatus;
  bcpStatus = bcp_getStatus();

  TRACE_BMGR_API("bmgr_execSendAck()\n");
  /* Check if a frame was received */
  if(BCP_STATUS_eIDLE != bcpStatus)
  {
    /* BCP still busy.
     * As we already received a valid BOOTSTRAP request we do not return to
     * state WAIT_CMD for possible transition to state ENTER_APP due to timeout,
     * thus it is not necessary to decement the timeout variable here.
     */
  }
  else
  {
...
void bmgr_execSendNak(T_BMGR_DATA* bmgrData)
{
  T_BCP_STATUS bcpStatus;
  bcpStatus = bcp_getStatus();

  TRACE_BMGR_API("bmgr_execSendNak()\n");
  if(bmgrData->timeout == 0)
  {
    /* Timeout exceeded.
	 * We will return to state WAIT_CMD for further transition to ENTER_APP as
     * soon as the tansmission of the response frame has finished.
     */
  }
  /* Check if a frame was received */
  if(BCP_STATUS_eIDLE != bcpStatus)
...
void bmgr_execEnterFbl(T_BMGR_DATA* bmgrData)
{
//  TRACE_BMGR_API("BMGR: bmgr_execEnterFbl\n");
  bmgr_exit();
  fbl_enter();
}
...
void bmgr_execEnterApp(T_BMGR_DATA* bmgrData)
{
  T_STATUS result;
  uint32 imageSize;

  TRACE_BMGR_API("BMGR: bmgr_execEnterApp()\n");
...
//   TRACE_BMGR_API("BMGR: bmgr_run()\n");
   ...
   case BMGR_STATE_eWAIT_CMD:
      bmgr_execWaitCmd(bmgrData);
      break;
   
   case BMGR_STATE_eSEND_ACK:
      bmgr_execSendAck(bmgrData);
      break;

   case BMGR_STATE_eSEND_NAK:
      bmgr_execSendNak(bmgrData);
      break;

   case BMGR_STATE_eENTER_FBL:
      bmgr_execEnterFbl(bmgrData);
      break;

   case BMGR_STATE_eENTER_APP:
      bmgr_execEnterApp(bmgrData);
      break;
