...
void fbl_sendSwInfoRsp(void)
{
  ...
  if(STATUS_eOK != bcp_allocTxPdu(&rspPdu))
  {
...
static void fbl_execRecvReq(T_FBL_DATA* fblData)
{
  ...
  if(BCP_ERR_ID_eNONE != errCode)
  {
    ...
    fblData->state = FBL_STATE_eSEND_RSP;
    TRACE_FBL_STATE("FBL: RECV_REQ -> SEND_RSP\n");
  }
  /* No error */
  else if(NULL == reqMsg)
  {
  ...
  else if(FBL_MSG_ID_eRESET_REQ == msgType)
  {
    /* Reboot command */
    fblData->state = FBL_STATE_eEXIT;
    TRACE_FBL_STATE("FBL: RECV_REQ -> EXIT\n");
  }
  else
  {
    /* Other command */
...
static void fbl_execSendRsp(T_FBL_DATA* fblData)
{
  T_BCP_STATUS bcpStatus;
  ...
  else
  {
    ...
    fblData->state = FBL_STATE_eRECV_REQ;
    TRACE_FBL_STATE("FBL: SEND_RSP -> RECV_REQ\n");
...
static void fbl_execExit(T_FBL_DATA* fblData)
{
  T_BCP_STATUS bcpStatus;
  ...
  else
  {
    ...
    TRACE_FBL_STATE("FBL State: EXIT -> REBOOT\n");
  }
}
...
static void fbl_execReboot(T_FBL_DATA* fblData)
{
  T_ROM_API* romApi;
  uint32 arg;
...
void fbl_run(void)
{
  ...
  {
  case FBL_STATE_eRECV_REQ:
    fbl_execRecvReq(fblData);
    break;

  case FBL_STATE_eSEND_RSP:
    fbl_execSendRsp(fblData);
    break;
    
  case FBL_STATE_eEXIT:
    fbl_execExit(fblData);
    break;

  case FBL_STATE_eREBOOT:
    fbl_execReboot(fblData);
    break;
...
void fbl_enter(void)
{
  ...
  fblData->state = FBL_STATE_eRECV_REQ;
  TRACE_FBL_STATE("FBL: INIT -> RECV_REQ\n");
